...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        N = query.shape[0]  # how many examples we send in at the same time
        value_len, key_len, query_len = values.shape[1], keys.shape[1], query.shape[1]

        # split embedding into self.heads pieces
        values = values.reshape(N, value_len, self.heads, self.head_dim)  # embed_size = heads * head_dim
        keys = keys.reshape(N, key_len, self.heads, self.head_dim)
        queries = query.reshape(N, query_len, self.heads, self.head_dim)

        energy = torch.einsum("nqhd, nkhd -> nhqk", [queries, keys])  # enisum --> 对向量 矩阵 张量的求和运算
        ...
        out = torch.einsum("nhqlm, nlhd -> nqhd", [attention, values]).reshape(N, query_len, self.heads * self.head_dim)
...
if __name__ == '__main__':
    ...
    trg = torch.tensor([[1, 6, 4, 2, 5, 9, 2, 0], [1, 5, 6, 2, 4, 7, 6, 2]]).to(device)
